...
{
    public class WAVSpeechStream : PullAudioInputStreamCallback
    {
        ...
        public WAVSpeechStream()
        {
            innerStream = new MemoryStream();
        }
        public WAVSpeechStream(WaveFormat WaveFormat)
        {
            ...
            if(innerStream!=null)
            {
                byte[] buffer = CreateWAVHeaderBuffer(0);
        ...
        public override int Read(byte[] buffer, uint size)
        {
            ...
            {
                innerStream.Position = readPosition;
                int red = innerStream.Read(buffer, 0, (int)size);
                readPosition = innerStream.Position;
